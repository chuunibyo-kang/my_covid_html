...
def visual_confirm_now_data_map():
    ...
    .set_global_opts(
        title_opts=opts.TitleOpts(title=" 全国现有确诊可视化地图（无澳门数据）",pos_left="center", pos_top="10%"),
        visualmap_opts=opts.VisualMapOpts(
            is_show = False,
            range_color = ['#ffffff','#ffe4d9','#ff907c','#ff665a','#fb1b30','#ca0000'],
                            max_ = 300),
...
def recent_mainland_overall_confirm_data_line_map() -> Line:
    ...
    .set_global_opts(
        title_opts=opts.TitleOpts(title="近期本土累计数据趋势（不包含港澳台）"),
        legend_opts = opts.LegendOpts(is_show = False))
    )
    return line_map
...
def total_confirm_top5_mainland_data_map() -> Bar:
    ...
    .add_yaxis("累计确诊人数",confirm_number)
    .set_global_opts(
        title_opts=opts.TitleOpts(title="本土累计确诊省份TOP5(不包含港澳台)"),
        legend_opts = opts.LegendOpts(is_show = False))
    )
    return bar_map   
...
def today_confirm_add_top5_data_map() -> Bar:
    ...
    .set_global_opts(
        title_opts=opts.TitleOpts(title="全国今日新增确诊省份TOP5（包含港澳台）"),
        legend_opts = opts.LegendOpts(is_show = False))
    )
    return bar_map
...
def today_confirm_add_top5_mainland_data_map() -> Bar:
    ...
    .add_yaxis("新增确诊人数", confirm_add_number)
    .set_global_opts(
        title_opts=opts.TitleOpts(title="本土今日新增确诊省份TOP5(不包含港澳台)"),
        legend_opts = opts.LegendOpts(is_show = False))
    )
    return bar_map    
